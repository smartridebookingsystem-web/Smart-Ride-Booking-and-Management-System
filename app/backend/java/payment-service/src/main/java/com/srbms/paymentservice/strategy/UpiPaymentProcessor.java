...
public class UpiPaymentProcessor implements PaymentProcessorStrategy {
    ...
    public Payment process(Payment payment, PaymentRequestDTO requestDTO) {
        ...
        payment.setGatewayRef("UPI_GW_" + UUID.randomUUID().toString().substring(0, 8).toUpperCase());
    ...
    public Payment refund(Payment payment, BigDecimal amount, String reason) {
        ...
        payment.setGatewayRef("UPI_RFD_" + UUID.randomUUID().toString().substring(0, 8).toUpperCase());
        return payment;
    }
}
